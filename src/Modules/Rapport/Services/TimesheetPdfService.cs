...
public class TimesheetPdfService
{
    ...
    public async Task<(byte[] Bytes, string FileName)> GenerateAsync(TimesheetExportRequestDto request)
    {
        ...
        var document = new TimesheetDocument(timesheet, company);
    ...
    private sealed class TimesheetDocument : IDocument
    {
        ...
        public TimesheetDocument(TimesheetDto timesheet, Company company)
        {
        ...
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                {
                    text.Span($"Erstellt am {DateTime.Now:dd.MM.yyyy}").FontSize(8).FontColor(Colors.Grey.Medium);
                });
            });
        }
        ...
        private void ComposeHeader(IContainer container)
        {
            container.Row(row =>
            {
                row.RelativeItem(1).AlignLeft().AlignMiddle().Height(50).Element(logo =>
                {
                    ...
                    {
                        logo.Text(_company.DisplayName).FontSize(14).Bold();
                    }
                });

                row.RelativeItem(2).AlignRight().AlignMiddle().Text(text =>
                {
                    text.Line(_timesheet.Title).FontSize(18).Bold();
        ...
        {
            return container.DefaultTextStyle(x => x.SemiBold()).PaddingVertical(4).BorderBottom(1).BorderColor(Colors.Grey.Lighten2);
        }
